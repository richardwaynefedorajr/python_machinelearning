...
def plotKNN(input_data, X_test, y_test, y_hat, optimal_k):
    ...
    fig.tight_layout()
    plt.suptitle('Histograms of iris dataset features', y=1.05)
    ...
    plt.tight_layout()
    plt.suptitle('Correlation matrix for iris dataset features', y=1.05)
    ...
    plt.suptitle('Pair plots for features of iris dataset', y=1.05)
    ...
    plt.suptitle('KNN results: k = '+str(optimal_k)+' with accuracy: '+str(metrics.accuracy_score(y_test, y_hat)), y=1.05)
    plt.savefig('knn_pairplots_predicted.png')
    plt.show()