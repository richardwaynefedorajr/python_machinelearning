...
fig = plt.figure()
ax = plt.axes()
ax.scatter(X_train[:,0], X_train[:,1], alpha=0.25, s=100*X_train[:,3], c=y_train, cmap='viridis')
ax.scatter(X_test[:,0], X_test[:,1], marker='v', alpha=0.5, s=100*X_test[:,3], c=y_test, cmap='viridis')
ax.scatter(X_test[:,0], X_test[:,1], marker='+', s=100*X_test[:,3], c=y_hat, cmap='viridis')


ax.set_title('Numpy logistic regression')
ax.set_xlabel(iris_data.feature_names[0])
ax.set_ylabel(iris_data.feature_names[1])
legend_elements = [Line2D([0], [0], marker='o', color='w', label='Training data', markerfacecolor='k', markersize=10),
                   Line2D([0], [0], marker='v', color='w', label='Test data', markerfacecolor='k', markersize=10),
                   Line2D([0], [0], marker='P', color='w', label='Predictions', markerfacecolor='k', markersize=10)]
