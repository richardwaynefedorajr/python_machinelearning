...
## 
## Code synopsis: this file contains a python function to reduce the width of an image and its corresponding minimum energy map by one pixel by removing pixels at seam indices
##
## Inputs  - im: MxNx3 image of datatype uint8
##         - energyImage: 2D matrix of datatype double (output of energy_image)
## Outputs - reducedColorImage: 3D matrix of reduced width, datatype uint8
##         - reducedEnergyImage: 2D matrix of reduced width, datatype double

import numpy as np

def reduceWidth(im, energyImage, indices):
    im_copy = np.copy(im)
    ...
    G_p = np.copy(im[:,:,1])
    B_p = np.copy(im[:,:,2])
    reducedColorImage = np.zeros((n_rows,n_cols-1,n_slices),dtype=np.uint8)
    reducedEnergyImage = np.zeros((n_rows_ei,n_cols_ei-1))
